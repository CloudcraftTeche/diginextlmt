...
const PORTFOLIO_DATA: WorkItem[] = [
  {
    ...
    description: "Helping a reputed coffee chain go digital with a powerful app in their Kuwait chapter",
    category: "Mobile App",
    industry: "Food",
    mediaSrc: "https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=800&h=600&fit=crop",
  ...
  },
  {
    ...
    description: "Delectably designed branded E-commerce store to launch a new sales channel",
    category: "E-commerce",
    industry: "Retail / Food",
    mediaSrc: "https://images.unsplash.com/photo-1555421689-d68471e189f2?w=800&h=600&fit=crop",
  ...
  },
  {
    ...
    category: "Web Design",
    industry: "Automobile",
    mediaSrc: "https://images.unsplash.com/photo-1486262715619-67b85e0b08d3?w=800&h=600&fit=crop",
  ...
  },
  {
    ...
    description: "Building a one-click path to groceries with an intensive CRM integration",
    category: "Mobile App",
    industry: "Retail",
    mediaSrc: "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=800&h=600&fit=crop",
  ...
  },
  {
    ...
    description: "An interactive-first approach to your timeless jewelry experience",
    category: "Branding",
    industry: "Retail / Fashion",
    mediaSrc: "https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=800&h=600&fit=crop",
  ...
  },
  {
    ...
    category: "Mobile App",
    industry: "Media",
    mediaSrc: "https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=800&h=600&fit=crop",
...
const PortfolioShowcase: React.FC = () => {
  ...
  // Extract unique industries and expertises
  const industries = ["All", ...Array.from(new Set(PORTFOLIO_DATA.map(item => item.industry)))];
  const expertises = ["All", ...Array.from(new Set(PORTFOLIO_DATA.map(item => item.category)))];

  // Filter works
  const filteredWorks = PORTFOLIO_DATA.filter(item => {
    const matchesIndustry = selectedIndustry === "All" || item.industry === selectedIndustry;
    const matchesExpertise = selectedExpertise === "All" || item.category === selectedExpertise;
    return matchesIndustry && matchesExpertise;
  });

  // Handle card click
  const handleWorkClick = (slug: string) => {
    router.push(`/work/test-slug`);
  };
    ...
    <section className={`${SECTION_PX} ${SECTION_PY} bg-white`}>
      <div className={CONTENT_WRAPPER_CLASSES}>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
          <div>
            <h1 className={`${SECTION_HEADING_SIZE} ${FONT_WEIGHT.semibold} mb-2`}>
              Case Studies
              <span className="text-gray-400 ml-3 text-xl">({filteredWorks.length})</span>
            </h1>
          </div>
          ...
          {filteredWorks.map((work) => (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                {/* Description */}
                <p className={`${DESCRIPTION_SIZE} text-gray-600 leading-relaxed`}>
                  {work.description}
                </p>
              </div>
            </motion.article>
        ...
        {filteredWorks.length === 0 && (
          <div className="text-center py-20">
            <p className="text-xl text-gray-400">No case studies found matching your filters.</p>
            <button
              onClick={() => {
