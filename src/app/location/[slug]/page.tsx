...
const LOCATION_SLUGS = Object.keys(locationsData);
...
export async function generateMetadata({
  ...
  return {
    title: data.title,
    description: data.content[0].substring(0, 160),
    keywords: `IT solutions ${data.city}, IT company ${data.city}, managed IT services ${data.city}`,
    openGraph: {
      title: data.title,
      description: data.content[0].substring(0, 160),
      url: `https://www.diginext.ae/location/${slug}`,
      images: data.mediaType === "image" ? [data.mediaUrl] : undefined,
    },
  };
}
